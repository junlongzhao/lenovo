...
class LSTMAttention(torch.nn.Module):
    def __init__(self, opt):
      ...
      # self.word_embeddings.weight = nn.Parameter(opt.embeddings, requires_grad=opt.embedding_training)
        ...
        self.bilstm = nn.LSTM(opt.embedding_dim, opt.hidden_dim // 2, batch_first=True, num_layers=self.num_layers,
    ...
    def attention(self, rnn_out, state):
        print("state_size",state.size())  #[layers,batchsize,embeding_size/2]
        merged_state = torch.cat([s for s in state],1)
        print("merged_state_size",merged_state.size()) #[batch,embed_size]
    ...
    def forward(self, X):
        ...
        print("rnn_out_size",rnn_out.size()) #[batch,seq,embedding_size]
        h_n, c_n = hidden
        print("h_n_size",h_n.size()) #[layers,batchsize,embeding_size/2]

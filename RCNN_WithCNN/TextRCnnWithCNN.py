...
class TextCNN_with_RCNN:
    def __init__(self, filter_sizes, num_filters, num_classes, learning_rate, batch_size, decay_steps, decay_rate,
        ...
        self.input_y = tf.placeholder(tf.float32, [self.batch_size, self.num_classes], name="input_y")  # y:[None,num_classes]
    ...
    def accuracy(self,logits,input_y_multilabel):
        result1 = tf.argmax(logits,axis=-1)
        result2 = tf.argmax(input_y_multilabel, axis=-1)
        #返回最大值，以及两个最大值的下标
